...
import { WalletTronInjectObjectNotFound } from "./error";
...
export class TronLinkAdapter extends EventEmitter {
    ...
    for (let i = 0; i < retries; i++) {
      ...
      if (w?.tronLink) {
        ...
        if (!w.tron && w.tronLink) {
          console.warn(
            "⚠️  Falling back to window.tronLink — some TIP‑1193 features may be missing."
          );
        }
        return;
      }
    ...
    try {
      ...
      const err_message =
        err && typeof err === "object" && "message" in err
          ...
          : "Failed to connect to TronLink wallet";
  ...
  async sendTransaction(rawTx: object): Promise<string> {
    ...
    try {
      ...
      if (!res?.txid)
        throw new WalletSignTransactionError("Transaction failed to send", res);
      return res.txid;
    } catch (err) {
  ...
  async switchChain(chainId: string): Promise<void> {
    ...
    try {
      if (!window.tron) {
        throw new WalletTronInjectObjectNotFound(
          "Tron object injection not found"
        );
      }
  ...
  private assertProvider(): asserts this is { provider: TronProvider } {
    if (!this.provider) {
      throw new WalletDisconnectedError(
        "Tron provider not injected (window.tron or window.tronLink missing)"
      );
    }
  }
